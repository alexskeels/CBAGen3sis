...
get_divergence_factor <- function(species, cluster_indices, landscape, config) {
  ...
  for(i_x in 1:length(cluster_indices_order)){
    ...
    for(j_x in 1:length(cluster_indices_order)){
      ...
      divergence_matrix[i_x, j_x] <- (sum(mean_temp_i, mean_temp_j)/(2))
    }
  }
...
apply_evolution <- function(species, cluster_indices, landscape, config) {
  ...
  for(cluster_index in unique(cluster_indices)){
    ...
    sigma_bs <- mean(landscape$environment[cells,"temp"], na.rm=T)/10
    # evolve temperature and body size
    delta_t  <- abs(rnorm(1, mean = 0, sd = sigma_t))
    delta_bs <- abs(rnorm(1, mean = 0, sd = sigma_bs))
