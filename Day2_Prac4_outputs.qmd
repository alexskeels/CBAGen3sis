...
The first object we will look at today is the sgenesis.rds file. This file contains a summary of the simulation. Which we can plot (as we did yesterday) with the plot_summary function. This is the same 
object, that you would have in memory by running a simulation with run_simulation.
...
daf$island_start <- sapply(species_t_0, function(x) unique(x$traits[, "start_island"]))
```
...
In this part we are going to load in a data set from Skeels et al. (2022) *SystBiol* in which we simulated data with Gen3sis under four alternative models to test the evolutionary speed hypoithesis (ESH). The ESH hypothesizes that faster rates of evolution occurs in lineages from warm regions like the tropics because they are have higher mutagenesis from faster life hisories associated with warm temperatures and smaller body sizes. The four models were

-   M0 - the null. population divergence is independent of temperature and body size

-   M1 - environmental temperature drives rate of population divergence

-   M2 - body size drives the rate of population divergence

-   M3 - environmental temperature and body size drives the rate of population divergence
